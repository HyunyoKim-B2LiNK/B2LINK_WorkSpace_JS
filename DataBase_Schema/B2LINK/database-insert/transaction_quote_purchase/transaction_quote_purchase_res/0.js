var a = [
	{
		...
		, "_id$trade_terms" : 0
		, "_id$transaction_quote_purchase_req" : 0
		...
		, "items" : [
			...
			, "quote_purchase_res_no" : "QP20161130P01"
	...
	, {
		...
		, "_id$partner_buyer" : 0
		, "_id$trade_terms" : 0
		, "_id$transaction_quote_purchase_req" : 1
		, "date_create" : [ 2016, 11, 30, 21, 9, 55 ]
		, "items" : [
			{
				"_id$brand_basic" : 0, "_id$product_basic" : 0
		...
		, "logistic_fee" : { "price" : 100 , "_id$currency" : 0 }
		, "quote_purchase_res_no" : "QP20161130P02"
		, "shipping_address" : "서울특별시"
		, "total_supply_price" : { "price" : 400 , "_id$currency" : 0 }
	}
];
...
var db0 = db.getSiblingDB( "transaction_quote_purchase" );
	db0.transaction_quote_purchase_res.remove({});
var io;
...
for( ; i<iLen; ++i )
{
	...
	db0.exec( "transaction_quote_purchase_res$addDoc( " + JSON.stringify( io ) + " );" );
};
